...
*   **Where:** Frontmatter of `.astro` files (e.g., `src/pages/supabase-test.astro`).
*   **How:** Imports `supabase` client from `src/lib/supabase.ts`.
*   **Flow:**
...
*   **Where:** `src/pages/api/*.ts` (e.g., `src/pages/api/feedback.ts`).
*   **How:**
...
*   **Where:** `<script>` tags or Framework components (React/Vue/Svelte) inside `.astro` pages.
*   **How:**
    1.  Page loads.
